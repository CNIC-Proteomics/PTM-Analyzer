...
calculatePvalues <- function(obj) {
  ...
  for (integration in obj$integrationSelected) {
    ...
    #Mean Difference
    loginfo(paste0(integration, " - Calculating Mean difference"), logger=obj$jobID)
    mean_diff <- MeanDiff(eset, Target, x, integration)
    mean_sign <- ifelse(mean_diff > 0, 1, -1)
    #names(mean_sign) <- gsub("_dX_", "_log_", names(mean_diff))
    #pvalues_df_tmp <- cbind(pvalues_df_tmp, mean_diff)
